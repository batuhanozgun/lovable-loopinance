...
export class SubcategoryService extends BaseCategoryService {
  constructor() {
    super();
    this.logger = categoryMutationLogger.createSubLogger('Subcategory');
  }
  ...
  async createSubCategory(data: ICreateSubCategoryData): Promise<ISubCategory> {
    try {
      ...
      return this.handleDbError(error, 'Alt kategori oluşturma');
    }
  }
  ...
  async updateSubCategory(id: string, data: IUpdateSubCategoryData): Promise<ISubCategory> {
    try {
      ...
      return this.handleDbError(error, 'Alt kategori güncelleme', { subCategoryId: id });
    }
  }
  ...
  async deleteSubCategory(id: string): Promise<void> {
    try {
      ...
      this.handleDbError(error, 'Alt kategori silme', { subCategoryId: id });
    }
  }
}
